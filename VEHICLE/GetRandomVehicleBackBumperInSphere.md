...
Vehicle GET_RANDOM_VEHICLE_BACK_BUMPER_IN_SPHERE(float x, float y, float z, float Radius, int modelHash, int Flags, int VehicleToBeIgnored);
```
...
RegisterCommand('TestNative', function()
    ...
    local test = GetRandomVehicleBackBumperInSphere(coords.x, coords.y, coords.z, 6.0, model, 2, 0)
    Wait(1000)
    DeleteEntity(test)
end, false)
```
